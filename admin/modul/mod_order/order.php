...
switch($_GET[act]){
  // Tampil Order
  default:
    echo "<h2>Pemesanan</h2>
		  <input type=button value='Cetak'
		  onclick=\"window.location.href='modul/mod_laporan/cetak_pesanan.php';\">
          <table>
          <tr><th>no.order</th><th>nama kustomer</th><th>Alamat</th><th>Telp</th><th>tgl. order</th><th>jam</th><th>status</th><th>aksi</th></tr>";

    $tampil = mysql_query("SELECT * FROM orders,kustomer WHERE orders.id_kustomer=kustomer.id_kustomer AND orders.status_order='Diproses' ORDER BY id_orders DESC");
    ...
    while($r=mysql_fetch_array($tampil)){
      ...
      echo "<tr><td align=center>$r[id_orders]</td>
                ...
                <td>$r[status_order]</td>
		            <td><a href=?halaman=order&act=detailorder&id=$r[id_orders]>Detail</a></td></tr>";
      $no++;
    }
    echo "</table>";
  ...
  case "detailorder":
    ...
    if ($r[status_order]=='Diproses'){
        $pilihan_status = array('Diproses', 'Ditolak', 'Dikirim');
    }
    elseif ($r[status_order]=='Dikirim'){
        $pilihan_status = array('Terkirim', 'Batal');    
    }
    else{
    ...
    echo "<h2>Detail Pemesanan</h2>
          <form method=POST action=$aksi?halaman=order&act=update>
          ...
          <select name='petugas'>
            ...
            $tampil=mysql_query("SELECT * FROM petugas ORDER BY nama_petugas");
            while($p=mysql_fetch_array($tampil)){
              echo "<option value=$p[nama_petugas]>$p[nama_petugas]</option>";
            }
    echo "</select></td></tr>
