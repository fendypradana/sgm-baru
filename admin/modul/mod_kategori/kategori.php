...
switch($_GET[act]){
  ...
  default:
    echo "<h2>Kategori</h2>
          ...
          onclick=\"window.location.href='?halaman=kategori&act=tambahkategori';\">
          <table>
    ...
    while ($r=mysql_fetch_array($tampil)){
       echo "<tr><td>$no</td>
             ...
             <td><a href=?halaman=kategori&act=editkategori&id=$r[id_kategori]>Edit</a> | 
	               <a href=$aksi?halaman=kategori&act=hapus&id=$r[id_kategori]>Hapus</a>
             </td></tr>";
      $no++;
    }
    echo "</table>";
  ...
  case "tambahkategori":
    echo "<h2>Tambah Kategori</h2>
          <form method=POST action='$aksi?halaman=kategori&act=input'>
          <table>
  ...
  case "editkategori":
    ...
    echo "<h2>Edit Kategori</h2>
          <form method=POST action=$aksi?halaman=kategori&act=update>
