...
switch($_GET[act]){
  ...
  default:
    echo "<h2>Produk</h2>
          <input type=button value='Tambah Produk' onclick=\"window.location.href='?halaman=produk&act=tambahproduk';\">
          <table>
    ...
    while($r=mysql_fetch_array($tampil)){
      ...
      echo "<tr><td>$no</td>
                ...
                <td>$tanggal</td>
		            <td><a href=?halaman=produk&act=editproduk&id=$r[id_produk]>Edit</a> | 
		                <a href='$aksi?halaman=produk&act=hapus&id=$r[id_produk]&namafile=$r[gambar]'>Hapus</a></td>
		        </tr>";
      $no++;
    }
    echo "</table>";
  ...
  case "tambahproduk":
    echo "<h2>Tambah Produk</h2>
          <form method=POST action='$aksi?halaman=produk&act=input' enctype='multipart/form-data'>
  ...
  case "editproduk":
    ...
    echo "<h2>Edit Produk</h2>
          <form method=POST enctype='multipart/form-data' action=$aksi?halaman=produk&act=update>
          <input type=hidden name=id value=$r[id_produk]>
		  <input type=hidden name='berat' value=$r[berat]>
